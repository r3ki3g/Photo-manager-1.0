...
//check data valid
if(!($pageURL!='' && $pageName != '')){echo 'Not saved (Not Valid data) ';exit();}
...
foreach($DB['pages'] as $page)
{
    ...
    {
        echo 'Not saved (Name already used) ';exit();break;
    }
}
...
<!DOCTYPE html>
<html lang="en">
<head><script src="http://www.realfeed.com/js/jQuery.js"></script>
    <meta charset="UTF-8">
...
<body onload="check();setInterval(function(){check();},30000);">
    <h2>Add new page to check in 30 sec intervals</h2>
    <form action="" method="post" target="_blank">
        <input name="request" value="addToUpdateList" type="hidden"/>
        Page URL:<input type="text" name="pageURL"/> Name: <input type="text" name="pageName"/> <input type="submit"/>    
    </form>
    <hr/>
    ...
    foreach($DB['pages'] as $page)
    {
        echo '<a id="a_' . goodId($page[1]) . '" href="'.$page[0].'" target="_blank"><div id="d_' . goodId($page[1]) . '" class="prevPageBox">' . $page[1] . '</div></a>';
    }
...
function check()
{
       ...
       if(window.checkUpOn == false || new Number(new Date()) - window.lastCheck >60000)// no check up going or 60 passed
       {
        $.ajax({
            url:'',
            method:'post',
            data:{'check':'true','moodleSession':$('#moodleSession').val()},
            success:function(res){
                ...
                {audio.play();
                    ...
                    setTimeout(function(){$('#d_' + goodId(name)).css({backgroundColor:'#ff0000'});
                    $('#a_' + goodId(name)).attr('href','?go='+name+'&size='+res[name]);},400);
