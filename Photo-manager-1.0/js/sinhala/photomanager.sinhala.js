...
$(document).ready(function() {
    ...
    function dragDropDetected1(data) {
        ...
        $("#dragDropBoxPortal").html('<div id="dragDropBox"><div id="popHead">ඡායාරූප මෙතැනට ඇදගෙනවිත් අත හරින්න</div>\
        <textarea  id="dragDropEater" placeholder="ඡායාරූප මෙතැනට අත හරින්න!"></textarea></div>');
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    if (status != "success") {
                        ...
                        $("#addPreviews").prepend('<div id = "_' + myId + '" class="previewImgBox"><img ondblclick="toggleFavourite(\'' + photoId + '\')" id = "prevImg_' + myId + '" class="' + className + ' prevImgElem' + additionalClassStr + '"  data-real-height="' + h + '" data-real-width="' + w + '" src="' + fileURL + '"/><div class="imageRemoveButton" onclick="removePhoto(' + myId + ');">එපා</div> <div class="fullSizeButton"><span href="' + fileURL + '" target="_blank" style="text-decoration:none;" onclick="zoomPreviewPhoto(\'prevImg_' + myId + '\');">ලොකුවට</span></div>\
                        \
                        \
                        <div style="display:none;" class="favouritedIconPreview" id="favoPreviewIco_' + photoId + '" data-title="ප්‍රියතම ඡායාරූපයකි : ඉන් ඉවත් කිරීමට දෙවරක් ඔබන්න"></div><div class="captionForEach"><textarea class="caption_text" id="caption_' + photoId + '" placeholder="පහදන වැකියක් දමන්න..."></textarea></div></div>');
            ...
            success: function(res) {
                ...
                if (search.query == val) {
                    ...
                    if (status == "success") {
                        ...
                        var addPeopleCode = '<div id="addPeopleRow" onclick="addNewGuy(\'' + res.query + '\');"> + <span class="peoplename">' + res.query + '</span>ව නම් ලැයිස්තුවට එක් කරන්න</div>';
...
function addNewGuy(name) {
    ...
    $('#popUpPortal').html('<div id="popUpBox"><div id="popHead">අලුත් කෙනෙක් ඇතුලත් කිරීම</div><div id="popCont">\
    <div id="newNameBox"><input type="text" id="newName" placeholder="නම" value="' + name + '"/></div>\
    <div id="otherNamesBox"><textarea id="otherNames"  value="" placeholder="වෙනත් නම් (සෙවීම් වලදී උපකරී වේ) පේලි වල වෙනමම දමන්න"></textarea></div>\
    <div id="photoNewPersonBox" onclick="selectFace();"><span id="selectPhotoAdvice">මුහුන සහිත රූපයක් තෝරන්න<br/></span></div>\
    <button id="submiterNewGuy">ඇතුලත් කරන්න</button><button id="cancelNewGuy" onclick="cancelNewGuy();">එපා</button>\
    </div></div>');
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                if (res.status == "success") {
                    ...
                    alert("ඇතුලත් කිරීමට නොහැකි වුනා!");
                }
            }
...
function addToFeaturingList(id, name, dp) {
    ...
    if (featuringList.includes(id)) {
        alert("දැනටමත් සිටින්නන්ගේ ලැයිස්තුවේ සිටී");
        return false;
    }
...
function peopleFilterIfNeeded(peopleList) {
    ...
    if (peopleList.length > 0) {
        out = '<div id="peopleFilterButton_0" onclick="filterByPeopleSetUp()">ගිණුම් අනුව පෙරන්න</div>\
        <div id="hintToFilterBySearch">ඔබට ඕනෑ අය තෝරන්න</div>';
    }
    return out;
}
...
function proceedSearchResultFilterByPeople() {
    ...
    //************************************************************************************* 
    var filterActivatedOrNotStatus = (currentSelectedForPeopleFilter.length > 0) ? ' (පෙරීමෙන් පසු)' : '';
    var photoResultsHeadHtml = '<div class="searchSummaryLine">ඡායාරූප ' + filteredPhotoList.length + ' ක් සොයාගන්නා ලදි' + filterActivatedOrNotStatus + '</div>';
   //zero case
    if(filteredPhotoList.length ==0){photoResultsHeadHtml = '<div class="searchSummaryLine">පෙරීමෙන් පසු ඡායාරූප නැත</div>';}
    ...
    for (i in filteredPhotoList) {
        ...
        var thisFavoHtml = '<div class="favouritedIconPreview favoIconSearchresults" style="display:' + isFavouritedStyle + ';" id="favoIcon_' + photoPath + '" data-title="ප්‍රියතම ඡායාරූපයකි : ඉන් ඉවත් කිරීමට දෙවරක් ඔබන්න"></div>';
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                if (res && res.status == "success") {
                    ...
                    var peopleResultsHtml = '<div class="searchSummaryLine">ගිණුම් ' + peopleList.length + ' ක් සොයා ගන්නා ලදි'   + peopleFilterIfNeeded(peopleList) + '</div>';
                    //zero case
                    if(peopleList.length==0){peopleResultsHtml = '<div class="searchSummaryLine">ගිණුම් හමු නොවුනි'   + peopleFilterIfNeeded(peopleList) + '</div>';}
                    ...
                    var photoResultsHeadHtml = '<div class="searchSummaryLine">ඡායාරූප ' + photoList.length + ' ක් සොයාගන්නා ලදි' + '</div>';
                    //zero case
                    if(photoList.length==0){photoResultsHeadHtml = '<div class="searchSummaryLine">ඡායාරූප හමු නොවුනි' + '</div>';}

                    //speacial if both zero 
                    if(photoList.length==0&&peopleList.length==0){photoResultsHeadHtml = '<div class="searchSummaryLine">ඡායාරූපද හමු නොවුනි' + '</div>';}
...
function showMorePhotosSearch() {
    ...
    if (photoShownNextIndex + bulkLength < photoResultHtmlPostSet.length) // there is bulkLength(10) more photos
    {
        ...
        if (moreCount > 0) {
            $('#showMorePhotos').html('තව ' + moreCount + 'ක් තිබේ');
        } else {
            noMoreSearchPhotos();
        }
...
function showMorePhotosFiltered() {
    ...
    if (photoFilteredShownNextIndex + bulkLength < photoResultHtmlPostSetFiltered.length) // there is bulkLength(10) more photos
    {
        ...
        if (moreCount > 0) {
            $('#showMorePhotosFiltered').html('තව ' + moreCount + 'ක් තිබේ');
        } else {
...
function noMoreSearchPhotos() {
    $('#showMorePhotos').html('එපමණයි...තව නැහැ.');
...
function noMoreSearchPhotosFiltered() {
    $('#showMorePhotosFiltered').html('එපමණයි...තව නැහැ.');
...
function duplicateWarning(fileNameRecieved) {
    ...
    $('#dupplicateDitectedPortal').html('<div id="duplicateWarningBox">\
        <div id="duplicatePopHead">එකම රූපය දෙවරක් උඩුගත වෙන්න යනවාදෝ!</div>\
        <div id="duplicatePopCont"><div class="fileNameDuplicateBox">' + fileNameRecieved + '</div> යන නම සහිත රූපයක් කලින් ද ඔබ උඩුගත කර ඇත.සමහර විට ඔබ දෙවරක් උඩුගත කරනවා විය හැක.එනිසාවෙන් එම රූපය රතු පාට මායිමෙන් සලකුනු කරනු ලැබුවෙමු.</div>\
        <div id="okayStringDuplicate" onclick="clearDuplicateWarning()">වැටහුනා</div>\
    </div>');
...
function RemoveFavoAskConfirm(id, postId) {
    $('#popUpPortal').html('<div id="removeFavoAskingPopBox">\
        <div id="removeFromFavoAskConfirmHead">ප්‍රියතම එකතුවෙන් ඉවත් කිරීම තහවුරු කරන්න</div>\
        <div id="removeFavoAskPopCont">ඔබට මෙම රූපය ප්‍රියතම එකතුවෙන් ඉවත් කිරීමට අවශ්‍ය බව නිසැකද?</div>\
        <div id="okayRemoveFromFavo" style="left: 275px;" onclick="RemoveFavo(\'' + id + '\', \'' + postId + '\');hideRemoveFromFavoConfirmBoxAndDimmer()">නිසැකයි.ඉවත් කරන්න</div>\
        <div id="cancelRemoveFromFavo" style="left:273px;" onclick="hideRemoveFromFavoConfirmBoxAndDimmer();">ඉවත් නොකරන්න</div>\
    </div>');
        ...
        success: function(res) {
            ...
            if (res) //valid json
            {
                if (res.status == "success") {
                    ...
                    $("#favoIcon_" + id).hide();
                    //give a message
                    showToast('ප්‍රියතම වලින් ඉවත් කෙරුනා!');
            ...
            success: function(res) {
                ...
                if (res) //valid json
                {
                    if (res.status == "success") {
                        ...
                        //give a message
                        showToast('ප්‍රියතම වලට එක් කෙරුනා!');
                    }
                }
            }
        });
...
//show random favourited photos at start
            ...
            if (res) //valid json
            {
                ...
                if (status == "success") //success
                {
                    ...
                    if (countOffavPhotosSent < 1) {
                        var noFavoYetHtml = '<div id="noFavoYetReminder">No favourites Found<div id="smallNoteNoFavo">ප්‍රියතම එකතුවට එක් කිරීමට දෙවරක් ඔබන්න</div></div>';
...
function setProfileOptionToNotSet() {
    $(".photoResultfeaturingDp[data-profileid][data-profile-option-set!=true]").click(function() {
        ...
        var profileOptionHtml = '<div data-title="<nothing>" data-profileid="' + id + '" data-photoid="' + photoId + '" data-photo-type="' + photoType + '" style="left:' + profileOptionPopBoxLeft + 'px;" class="profileOptionPopBox">ගිණුම් සැකසුම් පූරණය වෙමින්...</div>';
        ...
        success: function(res) {
            ...
            if (res) //valid json
            {
                ...
                if (status == "success") {
                    ...
                    var profileSettingHtml = '<div class="profileSettingsHead" ' + attrSet1 + '><img ' + attrSet1 + ' src="' + dpURL + '" data-title="ප්‍රතිරූපය මාරු කරන්න" class="dpPRofileSettingsHEad"/><input ' + attrSet1 + ' type="text" id="" class="editableNameProfileSettings" placeholder="නම සදහන් කරන්න" value="' + profile.name + '"/></div>\
                    ...
                    <div class="profileSettingsButtonRow">\
                        <div class="updateProfileSettingsButton" ' + attrSet1 + '>යාවත්කාලීන කරන්න</div>\
                        <div class="cancelProfileSettingsButton" ' + attrSet1 + '>අවලංගු </div>\
                    </div>\
                    </div>\
                    \
                    ';
...
function setProfilePhotoUpdateSelectPhoto() {
    $('.dpPRofileSettingsHEad').click(function() {
        ...
        $("#backgroundDimPortal").html('<div id="dimmer" data-onclick="$(\'#backgroundDimPortal\').html(\'\');$(\'#dragDropBoxPortal\').html(\'\');"></div>');
        $("#popUpPortal").html('<div id="POPdPuPDATERbOX"><div id="popHead">ඡායාරූපයක් තෝරන්න</div><div id="cancelProfileDpUpdateProccessIcon" onclick="cancelProfileDpUpdateProccess();">අවලංගු</div><div id="popCont">\
        <div id="profilePhotoUpdatePhotoSelectCont"><div id="loadingPhotosText0">ඡායාරූප පූරණය වෙමින්...</div></div>\
        \
        </div></div>');
...
function proPhotoUpdateSHowSuggPhotosInControl() {
    ...
    if (profilePhotoUpdateData.offset + bulkLength < profilePhotoUpdateData.count) //there is more than bulklength photos
    {
        ...
        var nextHtml = '<div id="showMoreProPicUpdateChooseImg" onclick="$(this).hide();proPhotoUpdateSHowSuggPhotosInControl();">තව පෙන්වන්න</div>';
    ...
    } else if (profilePhotoUpdateData.offset < profilePhotoUpdateData.count) // less than bulk length but therere are more photos
    {
        ...
        var noMorePhotosHtml = '<div id="noMorePhotosProPicUpdateSugg">එපමණයි...තව රූප නැහැ!</div>';
...
function selectToUpdatePropic(photoId) {
    ...
    $('#popHead').html('මුහුණ ඇති තැන පෙන්වන්න...නාසය මත ඔබන්න!');
    var photoURL = 'http:\/\/www.realfeed.com/photo.manager/' + photoId + '.jpeg';

    var selectFaceHtml = '<img src="' + photoURL + '" data-title="මුහුණ මත ඔබන්න" id="faceClickImage" onclick="clickOnFaceInDpUpdate(event,\'' + photoId + '\');"/>';
...
function clickOnFaceInDpUpdate(event, photoId) {
    ...
    $('#popHead').html('සුළු මොහොතක් රැදෙන්න');
    $('#proPicUpdatePhtoSuggContainer').html('<div id="waitingForDpSuggDPUpdate">සුදුසු ප්‍රතිරූප සැකෙසෙමින් පවතී.මොහොතක් සිටින්න</div>');
    setDataTitleToNotSet();
        ...
        success: function(res) {
            ...
            if (res) {
                if (res.status == 'success') {
                    ...
                    $('#popHead').html('ප්‍රතිරූපය ලෙස වඩාත් ගැලපෙන රූපය තෝරන්න');
...
function updateDpFinalConfirmed(thisElem) {
    ...
    $('.selectForDpImg').hide();
    $(thisElem).show();
    $('#settingIndicatorDpUpdtaeSelection').html('ප්‍රතිරූපය යාවත්කාලීන වෙමින්...');
    $('#popHead').html('යාවත්කාලීන වෙමින්');
        ...
        success: function(res) {
            ...
            if (res) {
                if (res.status == 'success') {
                    ...
                    $('#settingIndicatorDpUpdtaeSelection').html('<div id="dpUpdateSuccessMsg">සාර්ථකව යාවත්කාලීන වුනා!</div>');
                    setTimeout(function() {
                        $("#backgroundDimPortal").html('');
                        $('#popUpPortal').html('');
                        $('body').css('overflow', 'unset');
            ...
            success: function(res) {
                ...
                if (res) {
                    ...
                    if (status == 'success') {
                        ...
                        showToast('ඇතුලත් කළ නම් නැවත පරික්ෂා කරන්න...ඔබ ලබා දුන් නම් පිළිගත නොහැක!', 'formError');

                    }
                } else {
                    showToast('අපගේ වෙබ්සයිටයේ ගැටලුවක් ... යාවත්කාලීන කිරීම අසාර්තකයි!', 'formError');
                }
            }
        });
